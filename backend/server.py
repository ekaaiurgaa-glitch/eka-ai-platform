...
from services.job_card_manager import get_job_card_manager, JobStatus, JobPriority
from services.pdi_manager import get_pdi_manager, PDIStatus, PDICategory
from services.invoice_manager import get_invoice_manager, InvoiceStatus, InvoiceItemType
from services.ai_governance import get_ai_governance, GateType, GateResult
...
def chat():
    """Main intelligence endpoint with AI governance"""
    try:
        ...
        # Get user query from history
        user_query = ""
        if history and len(history) > 0:
            last_msg = history[-1]
            if last_msg.get('parts') and len(last_msg['parts']) > 0:
                user_query = last_msg['parts'][0].get('text', '')
        
        # ═══════════════════════════════════════════════════════════════
        # AI GOVERNANCE CHECK (4-Layer System)
        # ═══════════════════════════════════════════════════════════════
        ...
        # Build vehicle context for governance
        vehicle_context = {
            'registration_number': context.get('registrationNumber'),
            'brand': context.get('brand'),
            'model': context.get('model'),
            'year': context.get('year'),
            'fuel_type': context.get('fuelType')
        }
        
        governance_decision = governance.evaluate(
            query_id=str(uuid.uuid4())[:8],
            query=user_query,
            user_role=user_role,
            vehicle_context=vehicle_context,
            query_type='diagnostic' if status in ['CREATED', 'CONTEXT_VERIFIED', 'DIAGNOSED'] else 'general'
        )
        
        # Handle governance decision
        if governance_decision.final_action == 'BLOCK':
            return jsonify({
                "response_content": {
                    "visual_text": f"<div class='governance-block'>🛡️ {governance_decision.response_template}</div>",
                    "audio_text": governance_decision.response_template
                },
                ...
                "governance": governance_decision.to_dict()
            })
        
        elif governance_decision.final_action == 'CLARIFY':
            return jsonify({
                "response_content": {
                    "visual_text": f"<div class='governance-clarify'>❓ {governance_decision.response_template}</div>",
                    "audio_text": governance_decision.response_template
                },
                "job_status_update": status,
                "ui_triggers": {
        ...
        elif governance_decision.final_action == 'ESCALATE':
            return jsonify({
                "response_content": {
                    "visual_text": f"<div class='governance-escalate'>📞 {governance_decision.response_template}</div>",
                    "audio_text": governance_decision.response_template
                },
                "job_status_update": status,
                "ui_triggers": {
        ...
        # Normalize and add governance info
        response = normalize_response(result, status)
...
@flask_app.route('/api/pdi/evidence/<evidence_id>/verify', methods=['POST'])
@require_auth()
def verify_pdi_evidence(evidence_id):
    """Verify PDI evidence (supervisor action)"""
    data = request.get_json() or {}
    
    manager = get_pdi_manager(supabase)
    
    success, result = manager.verify_evidence(
    ...
    if not success:
        return jsonify({'error': result.get('error', 'Failed to verify evidence')}), 400
...
def download_invoice_pdf(invoice_id):
    ...
    # Get workshop details (would normally come from database)
    workshop_details = {
...
def get_pricing_guidance():
    ...
    try:
        # If specific part requested, get range from catalog
        if part_code:
            result = supabase.table('parts_catalog')\
                .select('*')\
                .eq('workshop_id', g.workshop_id)\
        ...
        # General query - use AI for range guidance
        # This would integrate with the AI to provide range-based responses
        return jsonify({
            'guidance': {
                'type': 'GENERAL',
...
def get_mg_contract(contract_id):
    ...
    try:
        ...
        # Get vehicle logs for this contract
...
def governance_check():
    ...
    Can be called before main chat to pre-validate
    """
    data = request.get_json()
    if not data:
        return jsonify({'error': 'No data provided'}), 400
