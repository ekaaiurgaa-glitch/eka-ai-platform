#!/usr/bin/env python3
"""
EKA-AI Schema Deployment Helper
Creates a properly ordered SQL script for Supabase deployment
"""

import os

def create_deployment_sql():
    """Create a deployment-ready SQL script with proper table order"""
...
def save_deployment_sql():
    """Save the deployment SQL to a file"""
    sql = create_deployment_sql()
    
    output_path = "/app/backend/database/schema_deployment.sql"
    
    with open(output_path, 'w') as f:
        f.write(sql)
    
    print(f"✅ Deployment SQL created: {output_path}")
    print(f"   Size: {len(sql)} characters")
    print(f"   Lines: {sql.count(chr(10))} lines")
    
    return output_path

if __name__ == "__main__":
    print("="  * 80)
    print("  EKA-AI SCHEMA DEPLOYMENT SQL GENERATOR")
    print("=" * 80)
    
    output_file = save_deployment_sql()
    ...
    print("\n5. Verify:")
    print("   python3 /app/deploy_schema.py")
