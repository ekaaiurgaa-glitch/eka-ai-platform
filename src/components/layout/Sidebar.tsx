  ...
  return (
    <aside className="w-64 bg-[#fafaf9] border-r border-gray-200 flex flex-col h-screen flex-shrink-0">
      ...
      <div className="flex-1 overflow-y-auto py-2 px-3 space-y-1">
        <NavItem to="/" icon={<MessageSquare size={18} />} label="Chats" />
        <NavItem to="/projects" icon={<FolderOpen size={18} />} label="Projects" />
        <NavItem to="/artifacts" icon={<PenTool size={18} />} label="Artifacts" />
        <NavItem to="/settings" icon={<Code size={18} />} label="Settings" />
        ...
        <div className="space-y-1">
           {/* Placeholder Recents */}
           <div className="px-3 py-2 text-sm text-gray-600 truncate hover:bg-gray-100 rounded-lg cursor-pointer">Fortuner brake diagnosis</div>
           <div className="px-3 py-2 text-sm text-gray-600 truncate hover:bg-gray-100 rounded-lg cursor-pointer">MG Contract Analysis</div>
        </div>
      </div>
