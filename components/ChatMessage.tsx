  ...
  const renderContent = (content: string) => {
    ...
    return lines.map((line, i) => {
      ...
      if (mainPointerMatch) {
        ...
        // Special case for DTC Definition block
        if (title.toLowerCase().includes('dtc definition')) {
          return (
            <div key={i} className="flex items-center gap-2 mt-6 mb-3 p-3 bg-[#FF6600]/10 border border-[#FF6600]/30 rounded-lg">
              ...
              <span className="text-[#FF6600] font-black text-sm uppercase tracking-widest">{title}</span>
            </div>
          );
        }
