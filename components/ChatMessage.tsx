...
const ChatMessage: React.FC<ChatMessageProps> = ({ 
  ...
  const renderContent = (content: string) => {
    ...
    return lines.map((line, i) => {
        ...
        return (
          <div key={i} className="mt-6 mb-2">
            <div className="flex items-center gap-2">
              <div className="w-1 h-4 bg-[#f18a22] rounded-full"></div>
              <span className="text-[#f18a22] font-black text-xs uppercase tracking-wider">{title}</span>
            </div>
            <div className="mt-1 h-px w-full bg-[#262626]"></div>
          </div>
        );
      }
      ...
      if (subPointerMatch) {
        return (
          <div key={i} className="ml-4 mb-2 flex items-start gap-3 group/subpoint">
            <span className="text-[#f18a22] font-black text-[10px] mt-1 shrink-0">{trimmedLine.split('.')[0]}.</span>
      ...
      return <div key={i} className="mb-2 text-sm text-zinc-300 leading-relaxed font-medium">{formatPriceRanges(trimmedLine)}</div>;
    });
  };
        ...
        {part_display_query && (
           <div className="group relative overflow-hidden rounded-xl border border-white/5 bg-[#121212] flex items-center p-4 gap-4 transition-all hover:border-[#f18a22]/20">
             <div className="w-16 h-16 rounded-lg bg-zinc-900 overflow-hidden shrink-0 border border-white/5">
                <img src={`https://source.unsplash.com/featured/?${encodeURIComponent(part_display_query)}`} alt={part_display_query} className="w-full h-full object-cover opacity-70 group-hover:opacity-100 transition-opacity" onError={(e) => { (e.target as HTMLImageElement).src = 'https://images.unsplash.com/photo-1486262715619-67b85e0b08d3?auto=format&fit=crop&q=80&w=400'; }} />
             </div>
             <div className="flex flex-col">
  ...
  return (
    <div className={`flex w-full mb-8 ${isAi ? 'justify-start' : 'justify-end'}`}>
      <div className={`max-w-[92%] md:max-w-[85%] p-6 rounded-2xl border shadow-2xl relative transition-all duration-300 ${isAi ? message.validationError ? 'bg-[#1a0a0a] border-red-900/50 text-zinc-100' : `bg-[#0A0A0A] ${showOrangeBorder ? 'border-[#f18a22]' : 'border-[#262626]'} text-zinc-100` : 'bg-[#121212] border-[#f18a22] text-zinc-100'}`}>
        {isAi && (
          <div className="absolute -top-3 -right-3 flex gap-2">
            {message.intelligenceMode === 'THINKING' && (
              <div className="text-[9px] px-3 py-1 rounded-full font-black uppercase tracking-[0.1em] shadow-2xl ring-4 ring-black bg-purple-600 text-white flex items-center gap-1.5 animate-pulse">
            ...
            {message.response_content?.audio_text && (
              <button onClick={() => onPlayAudio?.(message.response_content!.audio_text)} className={`p-2 rounded-full ring-4 ring-black shadow-2xl transition-all ${isAudioPlaying ? 'bg-[#f18a22] animate-pulse scale-110' : 'bg-zinc-800 hover:bg-[#f18a22] hover:scale-110'}`}>
            ...
            <div className={`text-[9px] px-3 py-1 rounded-full font-black uppercase tracking-[0.1em] shadow-2xl ring-4 ring-black ${message.validationError ? 'bg-red-600' : 'bg-green-600'}`}>{message.validationError ? 'Audit Breach' : 'Verified Output'}</div>
          </div>
        )}
        ...
        <div className="flex items-center justify-between mb-4 border-b border-white/5 pb-3">
          <div className="flex items-center gap-3">
            {isAi ? (
              <>
                ...
                <div className="flex flex-col">
                   <span className={`text-[11px] font-black uppercase tracking-widest ${message.validationError ? 'text-red-400' : 'text-[#f18a22]'}`}>eka-ai Service Governor</span>
                   <span className="text-[8px] text-zinc-500 font-bold uppercase tracking-tight">Governance: State Managed</span>
                </div>
              </>
            ) : (
              <div className="flex flex-col">
          ...
          <span className="text-[10px] text-zinc-600 font-mono font-bold bg-white/5 px-2 py-0.5 rounded">{message.timestamp.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
        </div>
        
        <div className="whitespace-pre-wrap font-medium">{isAi ? renderContent(displayContent) : <div className="text-zinc-200 text-sm leading-relaxed">{displayContent}</div>}</div>

        {isAi && renderVisualAssets()}

        {isAi && message.grounding_urls && message.grounding_urls.length > 0 && (
          <div className="mt-8 pt-4 border-t border-white/5">
            <h5 className="text-[10px] font-black text-zinc-500 uppercase tracking-[0.15em] mb-3 flex items-center gap-2"><span className="w-1 h-3 bg-zinc-700"></span>Verified Documentation & Suppliers</h5>
            <div className="flex flex-col gap-2">
              {message.grounding_urls.map((url, idx) => {
                ...
                return (
                  <a key={idx} href={url.uri} target="_blank" rel="noopener noreferrer" className={`text-[11px] px-4 py-3 bg-[#121212] border ${isPartSupplier ? 'border-blue-500/30 hover:border-blue-500' : 'border-[#262626] hover:border-[#f18a22]'} text-zinc-300 rounded-xl font-bold transition-all flex items-center justify-between group`}>
                    <div className="flex items-center gap-3">
                      <div className={`p-1.5 rounded-lg ${isPartSupplier ? 'bg-blue-600/20 text-blue-400' : 'bg-zinc-800 text-zinc-400'}`}>{isPartSupplier ? <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" /></svg> : <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" /></svg>}</div>
                      <span className="truncate max-w-[200px] md:max-w-md">{url.title}</span>
                    </div>
                    <svg className="w-4 h-4 opacity-0 group-hover:opacity-100 transition-opacity" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M17 8l4 4m0 0l-4 4m4-4H3" /></svg>
                  </a>
                );
              })}
            </div>
        ...
        {showContextForm && (
          <form onSubmit={handleContextSubmit} className="mt-8 p-6 bg-black/60 border-2 border-[#f18a22]/20 rounded-2xl animate-in fade-in zoom-in duration-500 shadow-inner">
            <h4 className="text-[11px] font-black text-[#f18a22] uppercase tracking-[0.25em] mb-6 flex items-center gap-3"><span className="w-2 h-2 bg-[#f18a22] rounded-full shadow-[0_0_8px_#f18a22]"></span>Critical: Identity Verification</h4>
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
              <div className="flex flex-col gap-1.5"><label className="text-[9px] font-black text-zinc-500 uppercase tracking-widest">Type</label><select name="vehicleType" required className="bg-[#121212] border border-[#262626] rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-[#f18a22] transition-all"><option value="">Select</option><option value="2W">2W</option><option value="4W">4W</option></select></div>
              <div className="flex flex-col gap-1.5"><label className="text-[9px] font-black text-zinc-500 uppercase tracking-widest">Brand</label><input name="brand" required placeholder="Honda" className="bg-[#121212] border border-[#262626] rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-[#f18a22] transition-all" /></div>
              <div className="flex flex-col gap-1.5"><label className="text-[9px] font-black text-zinc-500 uppercase tracking-widest">Model</label><input name="model" required placeholder="Civic" className="bg-[#121212] border border-[#262626] rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-[#f18a22] transition-all" /></div>
              <div className="flex flex-col gap-1.5"><label className="text-[9px] font-black text-zinc-500 uppercase tracking-widest">Year</label><input name="year" required placeholder="2019" className="bg-[#121212] border border-[#262626] rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-[#f18a22] transition-all" /></div>
              <div className="flex flex-col gap-1.5"><label className="text-[9px] font-black text-zinc-500 uppercase tracking-widest">Fuel</label><select name="fuelType" required className="bg-[#121212] border border-[#262626] rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-[#f18a22] transition-all"><option value="Petrol">Petrol</option><option value="Diesel">Diesel</option><option value="Electric">Electric</option></select></div>
            </div>
            <button type="submit" className="mt-6 w-full py-3 bg-[#f18a22] text-black text-[11px] font-black uppercase tracking-[0.2em] rounded-xl hover:bg-[#e55c00] transition-all shadow-xl">Initialize Context Synchronizer</button>
          </form>
        )}
      </div>
    </div>
